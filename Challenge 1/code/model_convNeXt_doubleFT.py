...
model_name = "model_convNeXt_doubleFT"
...
def train_model():
    ...
    ft_model.get_layer(network_keras_name).trainable = False
    ...
    N = 125
    N2 = 197
    # For i, layer in range N, N2, set layer.trainable = True
    for i, layer in enumerate(ft_model.get_layer(network_keras_name).layers[N:N2]):
    ...
    # Save the model
    ft_model.save("TransferLearningModel2")

    del ft_model

    # Re-load the model after transfer learning
    ft_model = tfk.models.load_model('TransferLearningModel2')
    ft_model.summary()
    ...
    # Compile the model
    ft_model.compile(loss=tfk.losses.BinaryCrossentropy(), optimizer=tfk.optimizers.Adam(1e-6), metrics='accuracy')
